...
> 약 43초 / 로그인 → 퀘스트 수락 → 진행 → 완료

https://github.com/user-attachments/assets/0c7f7703-3479-4493-b606-1359198e8e22

저장소 원본: [`docs/데이터기반_상호작용_데모_퀘스트_시연.mp4`](docs/데이터기반_상호작용_데모_퀘스트_시연.mp4)
...
- **수동 테스트 체크리스트**: [docs/TEST_CHECKLIST.md](docs/TEST_CHECKLIST.md)

## 데모/보안 (StartScene)

- **Google 로그인**: 기본적으로 비표시입니다(`showGoogleSignIn`, `showGoogleSignInInDemo`). 필요 시 인스펙터에서 활성화할 수 있습니다.
- **이메일 비밀번호 기억**: **에디터/Development 빌드에서만** UI/저장이 동작합니다. 릴리즈 빌드에서는 토글이 숨겨지며 Firebase Auth 세션만 사용합니다.
